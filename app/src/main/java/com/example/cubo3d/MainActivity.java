...
public class MainActivity extends AppCompatActivity {
    ...
    class ObjEsfera {
        float f12d3, objSize3;
        float rho3;
        float v113, v123, v133, v213, v223, v233, v323, v333, v433;
        Point3D[] f13w3;

        ObjEsfera(int numPuntos) {
            this.objSize3 = 2.0f; // Tamaño de la esfera
            this.rho3 = this.objSize3 * 2.0f; // Radio de la esfera
            this.f13w3 = new Point3D[numPuntos];

            for (int i = 0; i < numPuntos; i++) {
                double thetaRad = Math.toRadians(i * (360.0 / numPuntos));
                double phiRad = Math.toRadians(i * (180.0 / numPuntos) - 90.0);
                double x = Math.cos(phiRad) * Math.cos(thetaRad) * this.rho3;
                double y = Math.cos(phiRad) * Math.sin(thetaRad) * this.rho3;
                double z = Math.sin(phiRad) * this.rho3;
                this.f13w3[i] = new Point3D(x, y, z);
            }
        }
        void initPersp() {
            float costh = (float) Math.cos((double) this.theta3), sinth = (float) Math.sin((double) this.theta3);
            this.v113 = costh;
            this.v123 = 0;
            this.v133 = sinth;
            this.v213 = sinth * sinth - costh * costh;
            this.v223 = -2 * costh * sinth;
            this.v233 = sinth * costh + sinth * costh;
            this.v323 = sinth;
            this.v333 = -costh;
            this.v433 = -this.rho3;
        }
        ...
        void eyeAndScreen() {
            initPersp();
            for (int i = 0; i < 360; i++) {
                Point3D paint = this.f13w3[i];
                float z = (this.v133 * paint.f18x + this.v233 * paint.f19y + this.v333 * paint.f20z) + this.v433;
                this.vScr2[i] = new Point2D(((-this.f12d3) * (this.v113 * paint.f18x + this.v213 * paint.f19y)) / z,
                        ((-this.f12d3) * (this.v123 * paint.f18x + this.v223 * paint.f19y + this.v323 * paint.f20z)) / z);
            }
        }
    }
